...
これは、「フォームを編集する」のGeneralで回答結果を送信するメールアドレスとは別に設定可能です。
このmailtoオプションを使用すれば、通常のお問い合わせフォームと同様の入力画面が作成可能です。

<h2>１：セクションテキストに mailto を記述する</h2>

<li>フォームの編集にて、「新しいフィールド」をクリックします。
...
ノーの場合は、フォームの一般設定に指定したメールアドレスかＸＯＯＰＳの管理者アドレスが適用されます。
...
<h2>４：入力デフォルト値の設定</h2>
モジュール管理のフォーム管理タグより「フォームの結果を見る」にて回答者リストボックスから回答結果を見る事ができますが、
ここで「この投稿を入力初期値にセットする」をクリックするとデフォルトの入力値に設定できます。
また、解除は「フォームを編集する」からフォームを選びGeneralタブ項目Default Responseを空欄にします。
...
<p>
<li>フォームの編集'General'のEmail欄をメーリングリストの登録受付アドレスに指定します。また、From Option はアンケート中のemail欄とします。
<li>メールの送信元は、フィールドネーム'mailfrom'をテキスト・ボックスとして追加します。
