...
/*-----------------*/
include_once($FMXCONFIG['modpath'].'/include/function/form_render.php');
include_once($FMXCONFIG['modpath'].'/include/lib/espsql.php');
//include_once($FMXCONFIG['modpath'].'/include/lib/esphtml.forms.php');
...
if (!isset($_GET['key']) || !isset($_GET['sid'])){
	echo "Usage:<br> cast.php?key=[keycode]&sid=[Form ID]<br>";
...
//
$key = $_GET['key'];
$cast_key = formmakexTable::getXoopsModuleConfig('CAST_KEY');
$add_usage = formmakexTable::getXoopsModuleConfig('ADD_USAGE');
$cast_from = formmakexTable::getXoopsModuleConfig('MAILADDR');
...
$sql = 'select username,email,form_id from '.TABLE_RESPONDENT.' where disabled=2 '.$wstr.' order by form_id asc';
...
while(list($uname, $mailto, $sid) = $xoopsDB->fetchRow($db_result)){
	if($cur_sid<>$sid){
		$ret = form_render_email($sid,1,'',$hide_question);
		//$body = str_replace("\r\n", "<br>", $body);
		//echo $sid.$body;
		$cur_sid=$sid;
	}
...
if ($admin)
	redirect_header($FormCNF['admin'],3,"Form sent.");
