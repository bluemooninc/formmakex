...
if(isset($_POST['delete_ok'])) {
	...
	if($username){
		formmakexTable::delete_respondent($username);
	}
}
if(isset($_POST['update']) || isset($_POST['new']) || isset($_POST['delete'])) {
	...
	if($respondent['username']){
		formmakexTable::update_respondent($respondent);
	}
}
xoops_cp_header();
...
?>
	...
	<tr><td class="head" colspan="8"><b><?php echo _AM_FORMMAKEX_RESPONDENTS; ?></b></td></tr>
	<tr><td rowspan='12'valign="top" class='even' align='center' width='20%'>
		...
		<?php
		$sql = "SELECT * FROM ".TABLE_RESPONDENT." ORDER BY username,email ASC";
		...
		<p>
		<input type="submit" name="submit" value="<?php echo _AM_FORMMAKEX_EDIT;?>" />
		</form>
	</td>
<?php
	/*
	...
	*/
	...
	$opt['Y'] = $opt['N'] = "";
	if ($targetUnm){
		$respondent = formmakexTable::get_Respondentinfo($targetUnm);
	...
	echo "<center>"._AM_FORMMAKEX_RESPONDENT_USAGE."</center>";
	echo "<form method='post' action='respondent.php'>";
	echo "<tr><td class='odd' align='right'>"._AM_FORMMAKEX_USERNAME."</td><td class='even'>";
		if(isset($_POST['new'])) {
	...
	echo "</td></tr><tr>";
	echo "<td class='odd' align='right'>*"._AM_FORMMAKEX_PASSWORD."</td><td class='even'>";
	echo "<input type='text' name='password' value='".$password ."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_FNAME."</td><td class='even'>";
	echo "<input type='text' name='fname' value='".$fname."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_LNAME."</td><td class='even'>";
	echo "<input type='text' name='lname' value='".$lname."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_EMAIL."</td><td class='even'>";
	echo "<input type='text' name='email' value='".htmlspecialchars($email)."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_DISABLED."</td><td class='even'>";
	echo "<input type=radio name='disabled' value=1 ".$opt['Y'].">YES</input>";
	echo "<input type=radio name='disabled' value=2 ".$opt['N'].">NO</input>";
	echo "</td></tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_FORMID."</td><td class='even'>";
	echo "<input type='number' name='sid' value='".$sid."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>*"._AM_FORMMAKEX_RESPONSEID."</td><td class='even'>";
	echo "<input type='number' name='rid' value='".$rid."' />";
	if ( $sid && $rid ){
		$url = sprintf(XOOPS_URL."/modules/"
			.$xoopsModule->dirname()
			."/manage.php?where=results&sid=%u&rid=%u",$sid,$rid);
		echo "&nbsp;<a href='".$url."'>"._AM_FORMMAKEX_SEEARESULT."</a>";
	}
	echo "</td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>*"._AM_FORMMAKEX_CHANGED."</td><td class='even'>";
	echo "<input type='date' name='changed' value='".$changed."' /></td>";
	echo "</tr><tr>";
	echo "<td class='odd' align='right'>"._AM_FORMMAKEX_EXPIRE."</td><td class='even'>";
