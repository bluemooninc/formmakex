  ...
  <tr><td><ul>
    ...
    <li>ESP_BASE: <?php
        if ((ESP_BASE == dirname(__FILE__) . '/../') || (ESP_BASE == dirname(dirname(__FILE__)) .'/'))
            ...
            _fail(ESP_BASE);
    ?></li>
    <li>base_url: <?php check_string($FMXCONFIG['base_url'], 'http://' . $_SERVER['HTTP_HOST'] . dirname(dirname($_SERVER['REQUEST_URI'])) . '/'); ?></li>
    <li>Version: <?php _pass($FMXCONFIG['version']); ?></li>
    <li>Debug: <?php check_bool($FMXCONFIG['DEBUG'], false); ?></li>
  </ul></td></tr>

  <tr><th>phpESP Language Settings</th></tr>
  <tr><td><ul>
    <li>GNU Gettext: <?php check_string(
            ($FMXCONFIG['gettext'] ? 'Real' : 'Emulated'), 'Real'); ?></li>
    <li>default_lang: <?php _pass($FMXCONFIG['default_lang']); ?></li>
    <li>current lang: <?php _pass($FMXCONFIG['lang']); ?></li>
    ...
    <li>GNU Gettext test: <?php
        esp_setlocale('en_US');
        check_string( _MB_Gettext_Test_Failed , 'Passed'); ?></li>
    <li>Catalog Open Test: <?php
        $ret = fopen($FMXCONFIG['locale_path'] . '/en_US/LC_MESSAGES/messages.mo', 'r');
