...
if (!is_object($xoopsUser)) {
	redirect_header(XOOPS_URL.'/modules/'.$mydirname.'/',2,_MD_FORMMAKEX_CAN_WRITE_USER_ONLY);
	exit();
}
/*
...
elseif(isset($fmxEditForm->editInfo['form_id'])) $sid = $fmxEditForm->editInfo['form_id'];
...
switch($where){
	...
	case 'tab':
	case 'copy':
		if ( $formTable->editbyGroup()!=true && $uid!=$xoopsUser->uid()) $err = true;
		break;
}
if($err==true){	
	$xoopsTpl->assign('message', _MD_FORMMAKEX_YOU_DONT_HAVE_A_PERMISSION);
...
$FMXCONFIG['csv_charset']  = formmakexTable::getXoopsModuleConfig('CSV_CHARSET');
...
if ($where == 'download') {
	...
	$xoopsOption['template_main'] = 'formmakex_design.html';
