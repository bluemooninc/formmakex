...
include_once('./class/formmakexQuestionLoader.class.php');
$fmxRender = new formmakexHtmlRender();

//var_dump($fmxEditForm->editInfo);

if(empty($fmxEditForm->editInfo['curr_qNumber'])) $fmxEditForm->editInfo['curr_qNumber'] = 0;
if(empty($_POST['id'])) $_POST['id'] = 0;
// update failed, stay on same question
if( !isset($_POST['q'])){
	//$fmxEditForm->isUpdated();
	$curr_qNumber = $fmxEditForm->editInfo['curr_qNumber'];
}else{
...
$formQuestion = new questionLoader($sid);
...
if(isset($_POST['extra_choices']) && $curr_qNumber==0){
	$edit_qid = $fmxEditForm->InsertQuestion($form_id);
	$curr_qNumber = $fmxEditForm->getNewPosition($form_id) + 1;
}*/
...
$fmxEditForm->editInfo['curr_qNumber'] = $curr_qNumber;

$sql = "SELECT id, type FROM ".TABLE_QUESTION_TYPE." WHERE id != 99";
$result = $xoopsDB->query($sql);
$typeIdArr = array();
if ( !defined( FORMMAKEX_QTYPE_1) ){
	$lang_typeId = array(
		1   => FORMMAKEX_QTYPE_1,
		2   => FORMMAKEX_QTYPE_2,
		3   => FORMMAKEX_QTYPE_3,
		...
		8   => FORMMAKEX_QTYPE_8,
		9   => FORMMAKEX_QTYPE_9,
		10  => FORMMAKEX_QTYPE_10,
		40  => FORMMAKEX_QTYPE_40,
		99  => FORMMAKEX_QTYPE_99,
		100 => FORMMAKEX_QTYPE_100
	);
}
...
/*		'length' => array(
		...
		'content' => array(
			...
			'label' => FORMMAKEX_TABS_QUESTIONS_QUESTION_CONTENT
		)
	)
);

$tpl_vars['langs']['question_name'] = _MB_Field_Name;
$tpl_vars['langs']['append_question'] = _MB_New_Field;
$tpl_vars['langs']['append_choice'] = _MB_Add_another_answer_line;
$tpl_vars['langs']['question_infos'] = FORMMAKEX_TABS_QUESTIONS_QUESTION_INFOS;
$tpl_vars['langs']['choices_infos'] = FORMMAKEX_TABS_QUESTIONS_CHOICES_INFOS;
$tpl_vars['langs']['field_number'] = FORMMAKEX_TABS_QUESTIONS_FIELD_NUM;
$tpl_vars['langs']['field_control'] = FORMMAKEX_TABS_QUESTIONS_FIELD_CONTROL;
$tpl_vars['langs']['choices_description'] = FORMMAKEX_TABS_QUESTIONS_CHOICES_DESC;
