...
/* {{{ proto bool form_auth(int form_id, string username, string password)
   Returns true if the supplied username/password is allowed
   access to the form. */
function form_auth($sid, $username, $password) {
...
//    esp_init_db();
	...
	if ( $maxlogin > 0 ) {
		...
		$sql = "SELECT COUNT(*) < '$maxlogin' FROM ".TABLE_RESPONSE." WHERE form_id = '${sid}' AND complete = 'Y' AND username = '$username'";
...
function manage_auth($username, $password) {
	...
	if(isset($fmxEditForm->accessLevel) &&
	   isset($fmxEditForm->accessLevel['expired'])) {
		if($fmxEditForm->accessLevel['expired']-- > 0) {
			$username = ''; $password = '';
		}
	}
	...
	if (!empty($username) &&
			!empty($password) &&
			isset($fmxEditForm->accessLevel) &&
			isset($fmxEditForm->accessLevel['username']) &&
			isset($fmxEditForm->accessLevel['password']) &&
			$fmxEditForm->accessLevel['username'] == $username) {
		$sql = "SELECT PASSWORD('$password') = '". $fmxEditForm->accessLevel['password'] ."'";
		list($auth) = $xoopsDB->fetchRow($xoopsDB->query($sql));
...
function manage_logout() {
	...
	$fmxEditForm->accessLevel = array('expired' => 2);
}
/* }}} */
...
/* {{{ proto int auth_get_rid (int form_id, string username, int response_id)
   Returns the RID to use for this user. */
function auth_get_rid($sid, $username, $rid = 0) {
	$rid = intval($rid);
	if (isset($rid) && $rid != 0) {
		// check for valid rid
		$sql = "SELECT (r.username = '$username' && r.complete = 'N') AS valid FROM ".TABLE_RESPONSE." r WHERE r.id = '$rid' AND r.form_id = '$sid'";
		...
		$sql = "SELECT r.id FROM ".TABLE_RESPONSE." r WHERE r.form_id = '$sid' AND r.complete = 'N' AND r.username = '$username' ORDER BY submitted DESC LIMIT 1";
		$res = $xoopsDB->query($sql);
		if ($res && $xoopsDB->getRowsNum($res) > 0)
