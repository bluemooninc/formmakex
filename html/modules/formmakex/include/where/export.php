...
<?php
	...
	if($xoopsUser->isAdmin()) {
		...
		$realms = array_to_insql(
			array_intersect(
				$fmxEditForm->accessLevel['pall'],
				$fmxEditForm->accessLevel['pdata']));
...
<?php
	...
	if(isset($_GET['test'])) {
		...
		if($fmxEditForm->accessLevel['superuser'] != 'Y' &&
				...
				!in_array($srealm, array_intersect(
						$fmxEditForm->accessLevel['pdesign'],
						$fmxEditForm->accessLevel['pall'])) &&
	...
	if($type == 'dbf') {
		$file = $fmxEditForm->accessLevel['home'] . "/$name.dbf";
		...
		$file = $fmxEditForm->accessLevel['home'] . "/$name.csv";
