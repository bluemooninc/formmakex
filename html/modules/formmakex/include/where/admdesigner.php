...
// <jflemer@acm.rpi.edu>
	...
	if(isset($_POST['delete'])) {
		...
		if($xoopsUser->isAdmin() ||
			in_array($r, $fmxEditForm->accessLevel['pgroup']) ||
	...
	if(isset($_POST['submit'])) {
		...
		if($r == 'superuser' && $fmxEditForm->accessLevel['superuser'] != 'Y')
			$r = '';
		...
		if(!$result || $xoopsDB->getRowsNum($result) < 1) {
			if(empty($u) || empty($r) || empty($password)) {
				$errstr .= $formRender->mkerror(_MB_Username_are_required);
			} else {
...
<table border="0" cellspacing="0" cellpadding="4" align="center" bgcolor="<?php echo($FMXCONFIG['active_bgcolor']); ?>">
	<tr>
<?php
	echo("<th bgcolor=\"$bg2\" align=\"right\">". _MB_Username ."</th>\n");
	if(empty($u))
...
<?php
	...
	if(empty($r)) {
		...
		if($xoopsUser->isAdmin()) {
			...
			$groups =& $fmxEditForm->accessLevel['pgroup'];
		}
		foreach($groups as $g) {
