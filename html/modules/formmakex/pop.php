...
include_once($FMXCONFIG['modpath'].'/include/lib/espsql.php');
include_once($FMXCONFIG['modpath'].'/include/lib/espresponse.php');
...
$host = formmakexTable::getXoopsModuleConfig('MAILSERVER');
$user = formmakexTable::getXoopsModuleConfig('MAILUSER');
$pass = formmakexTable::getXoopsModuleConfig('MAILPWD');
$mail = formmakexTable::getXoopsModuleConfig('MAILADDR');
...
if ($num == "0") {
	...
	if (!$img_mode){
		if ($admin)
			redirect_header($FormCNF['admin'],2,$JUST_POPED.'(no email found)');
		else
			redirect_header($FormCNF['root'],2,$JUST_POPED.'(no email found)');
...
for($j=1;$j<=$num;$j++) {
	...
	foreach ($part as $multi) {
		...
		if (strtolower($main) == "text") {
			...
			if ($write) {
				...
				if (strlen($content) > $FormCNF['post_limit']) $content = substr($content, 0, $FormCNF['post_limit'])."...";
			}
		}else{
			...
			if (eregi($FormCNF['subtype'], $sub)){ $deny=0; } else { $deny=1; };
			$ext = end(explode(".",$filename)); //strtolower(
			if (eregi($FormCNF['viri'], $ext)) $deny = 1;			
			if (eregi("\nContent-Transfer-Encoding:.*base64", $m_head) && eregi($FormCNF['imgtype'].'|'.$FormCNF['embedtype'], $sub)) {
				$upfile_localname=$uname."_".time().$permission."_".$filename;		// convert for mbstrings
				$tmp = base64_decode($m_body);
				if (strlen($tmp) < $FormCNF['maxbyte'] && !eregi($FormCNF['viri'], $filename) && $write) {
					$upfile_localname = cnv_mbstr($upfile_localname);
					$upfile_url = XOOPS_URL.$FormCNF['img_dir'].rawurlencode($upfile_localname);
					$upfile_path = XOOPS_ROOT_PATH.$FormCNF['img_dir'].$upfile_localname;
					$fp = fopen($upfile_path, "wb");
					fputs($fp, $tmp);
					fclose($fp);
					$attach = $filename;
 					if (eregi($FormCNF['imgtype'], $sub)){
						// Thumbs Support ( PHP GD Libraly Required )
						if (eregi($FormCNF['thumb_ext'],$upfile_localname)) {
							$size = getimagesize($upfile_path);
							if ($size[0] > $FormCNF['w'] || $size[1] > $FormCNF['h']) {
								$thumbfilename = thumb_create($upfile_path,$FormCNF['w'],$FormCNF['h'],XOOPS_ROOT_PATH.$FormCNF['thumb_dir']);
								$addimg .= "[url=".$upfile_url."][img align=left]".XOOPS_URL.$FormCNF['thumb_dir'].rawurlencode($thumbfilename)."[/img][/url]";
							} else {
 					...
 					} elseif (eregi($FormCNF['embedtype'], $sub)){
						$addimg .= "\n<EMBED src=\"".$upfile_url."\" WIDTH=\"".$FormCNF['w']."\" HEIGHT=\"".$FormCNF['h'].
						"\" autostart=\"false\" controller=\"true\" hspace=\"5\" align=\"left\" alt=\"\"></EMBED>\n";
					}
				} else {
					$write = false; $notice ="Max upload size (".$FormCNF['maxbyte'].") Over or Deny Ext.($ext)";
				}
			} elseif ($deny==0) {
				$tmp = base64_decode($m_body);
				...
				$fp = fopen($FormCNF['uploads'].$upfile_localname, "wb");
				fputs($fp, $tmp);
				fclose($fp);
				$addfile .= "\n:download:[url=".$FormCNF['root']."download.php?url=".$upfile_url."]".$filename."[/url]\n";
				// $addfile .= $m_head.$sub;
			} elseif ( strlen($ext)>0 ) {
	...
	if ($write) {
		...
		// Save a header info.
		if ($head_save) form_head_save($id,$head);
	} else {
		// Save a deny mail log
		if ($denylog_save) form_deny_log($head,$subject,str_replace("<br />","\n",$content),$notice);
	}
}
// Logfile max line
if (count($lines) > $maxline) {
	for ($k=count($lines)-1; $k>=$maxline; $k--) {
		...
		if (file_exists($FormCNF['uploads'].$at)) @unlink($FormCNF['uploads'].$at);
		$lines[$k] = "";
	}
}
// Save to log.
...
//
if ($admin)
	redirect_header($FormCNF['admin'],3,$JUST_POPED);
else
	redirect_header($FormCNF['root'],12,$JUST_POPED);
exit;
/******************************************************************************/
...
function form_deny_log($head,$subject,$body,$notice){
	global $denylog;
...
function form_head_save($id,$head){
...
function member_ctl($l_stack, $from, $now) {
	...
	if( list($num)=$xoopsDB->fetchRow($xoopsDB->query($sql)) ){
		if ( $num == 0 ){
			$sql = sprintf("insert into %s(username,email,fname,lname,form_id,changed,expiration) values('%s','%s','%s','%s','%u','%s','%s')"
				,TABLE_RESPONDENT,$unm,$from,$fnm,$lnm,$sid,$sqlDate,$expDate);
		}elseif ( $num > 0 ){
			$sql = sprintf("update %s set email='%s',fname='%s',lname='%s',form_id='%u',changed='%s',expiration='%s' where username = '%s'"
...
function qreturn_ctl($cmd, $l_stack, $from, $now) {
	...
	$OneRes = formmakexTable::getXoopsModuleConfig('ONE_RESPONSE');
	$unm    = isset($l_stack['u']) ? $l_stack['u'] : "" ;	// User name
	$ticket = isset($l_stack['t']) ? $l_stack['t'] : "" ;	// Ticket
	$sid    = isset($l_stack['s']) ? $l_stack['s'] : 0 ;	// Form id
	...
	switch ($cmd){
		case "INP": //Input data.
			$sql = sprintf("select form_id,response_id from %s where username='%s' and password='%s'",TABLE_RESPONDENT, $unm, $ticket);
			...
			break;	// Request From with form name
		case "DEL": //Input data.
			$sql = sprintf("select form_id,response_id from %s where username='%s' and password='%s'",TABLE_RESPONDENT, $unm, $ticket);
			if ( list($r_sid,$rid)=$xoopsDB->fetchRow($xoopsDB->query($sql)) ){
				if ( !$r_sid || $r_sid!=$sid ) return _MD_POP_QINP_FAILEDLOGIN;
...
function ret_result_mail($to,$key,$cmd,$subject,$message='') {
	...
	$tempdir = $FormCNF['path']."language/".$xoopsConfig['language']."/mail_template/";
	...
	if ( function_exists('mb_encode_mimeheader') )
		$subj = mb_encode_mimeheader( $subj, formmakexTable::get_mailcode(), "B" );
